...
def test_extract_land_bbox_found(tmp_path, point_dest) -> None:
    ...
    with patch("src.category_c._LAND_SHP", land_shp):
        feat = extract_land_bbox(dest)

    assert feat is not None
    assert feat["properties"]["iso_a2"] == "BV"
...
def test_extract_land_bbox_missing_file(tmp_path, point_dest) -> None:
    """extract_land_bbox returns None when ne_10m_land.shp does not exist."""
    from unittest.mock import patch
